...
warn("[+] Solvexis Hub : anticheat bypassed.")
end)()
...
local Window = Library:CreateWindow({
	Title = "",
	Footer = "Solvexis Hub | VAULT$EN$E",
	Icon = 241778280,
	NotifySide = "Right",
...
if isOwner then
    Library:Notify("Owner Solvexis User! Welcome " .. plr.DisplayName .. " (@" .. plr.Name .. ")", 15)
end
if hasBypass then
...
tracerOutline.Color = solvexis_cfg.TargetAim.TracerOutlineColor
tracerOutline.Thickness = 4

local tracer = Drawing.new("Line")
tracer.Visible = false
tracer.Color = solvexis_cfg.TargetAim.TracerFillColor
tracer.Thickness = 2
...
local HitChams = LPH_NO_VIRTUALIZE(function(Player)
    if not solvexis_cfg.HitEffects.HitChams.Enabled then return end  -- Sử dụng solvexis_cfg từ script gốc
    if Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        for _, BodyPart in ipairs(Cloned:GetChildren()) do
            if BodyPart:IsA("BasePart") then
                ...
                BodyPart.Transparency = solvexis_cfg.HitEffects.HitChams.Transparency
                BodyPart.Color = solvexis_cfg.HitEffects.HitChams.Color
                BodyPart.Material = solvexis_cfg.HitEffects.HitChams.Material
            end
        end
        if Cloned:FindFirstChild("Head") then
            local Head = Cloned.Head
            Head.Transparency = solvexis_cfg.HitEffects.HitChams.Transparency
            Head.Color = solvexis_cfg.HitEffects.HitChams.Color
            Head.Material = solvexis_cfg.HitEffects.HitChams.Material
        ...
        local tweenInfo = TweenInfo.new(
            solvexis_cfg.HitEffects.HitChams.Lifetime,
        ...
        task.delay(solvexis_cfg.HitEffects.HitChams.Lifetime, function()
...
local HitChamsSkeleton = LPH_NO_VIRTUALIZE(function(Player)
    if not solvexis_cfg.HitEffects.HitSkeleton.Enabled then return end  -- Thêm HitSkeleton vào solvexis_cfg.HitEffects
    if Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        local bones = {
            {"Head", "UpperTorso"},
        ...
        for _, bonePair in ipairs(bones) do
            ...
            if parentBone and childBone then
                ...
                line.Transparency = solvexis_cfg.HitEffects.HitChams.Transparency  -- Sử dụng chung transparency
                line.Color = solvexis_cfg.HitEffects.HitSkeleton.Color  -- Sử dụng Color mới
                line.Material = Enum.Material.Neon
                line.Parent = workspace
                local tweenInfo = TweenInfo.new(solvexis_cfg.HitEffects.HitChams.Lifetime / 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
...
local function GetClosestCharacter()
    ...
    for _, player in pairs(players:GetPlayers()) do
        ...
        if not isAlive(player) and solvexis_cfg.Checks.Alive then continue end
        if solvexis_cfg.Checks.Team and player.Team == localPlayer.Team then continue end
        if solvexis_cfg.Checks.Forcefield and player.Character:FindFirstChildWhichIsA("ForceField") then continue end
        ...
        if solvexis_cfg.Checks.Wall then
...
local function createHitSound()
    ...
    sound.SoundId = solvexis_cfg.HitEffects.HitSoundID
    sound.Volume = solvexis_cfg.HitEffects.HitSoundVolume
    sound:Play()
...
TargetAimTab:AddToggle('TargetAimEnabled', {
    ...
    Callback = function(Value)
        solvexis_cfg.TargetAim.Enabled = Value
        if not Value then
            solvexis_cfg.TargetAim.Target = "None"
            tracer.Visible = false
            tracerOutline.Visible = false
    ...
    Mode = 'Toggle',
    Callback = function(Value)
        if not solvexis_cfg.TargetAim.Enabled then return end
        if Value then
            local target = GetClosestCharacter()
            if target then
                solvexis_cfg.TargetAim.Target = target.Name
            else
                solvexis_cfg.TargetAim.Target = "None"
            end
        else
...
if UserInputService.TouchEnabled then
    ...
    ImageButton.MouseButton1Click:Connect(function()
        if solvexis_cfg.TargetAim.Enabled then
...
TargetAimTab:AddToggle('AutoSelect', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.AutoSelect = Value
        if Value then
...
TargetAimTab:AddToggle('AutoFire', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.AutoFire = Value
    end
})
...
TargetAimTab:AddToggle('TargetStatsEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.TargetStats = Value
    end
...
local solvexis_cfg_TargetStats = Instance.new("ScreenGui")
...
solvexis_cfg_TargetStats.Name = "solvexis_cfg_TargetStats"
solvexis_cfg_TargetStats.Parent = game.CoreGui
solvexis_cfg_TargetStats.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Background.Name = "Background"
Background.Parent = solvexis_cfg_TargetStats
...
RunService.Heartbeat:Connect(function()
    if solvexis_cfg.TargetAim.TargetStats and solvexis_cfg.TargetAim.Enabled then
        local targetName = solvexis_cfg.TargetAim.Target
...
TargetAimTab:AddToggle('Highlight', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.Highlight = Value
    end
}):AddColorPicker('HighlightOutline', {
    Default = solvexis_cfg.TargetAim.HighlightOutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        solvexis_cfg.TargetAim.HighlightOutlineColor = Value
    end
})
...
local function connectHealth(target)
    ...
    healthConnections[target] = hum.HealthChanged:Connect(function(newHP)
        local highlight = target.Character and target.Character:FindFirstChild("SolvexisHighlight")
        ...
        task.delay(0.5,function()
            if highlight and solvexis_cfg.TargetAim.Highlight then
                highlight.OutlineColor = solvexis_cfg.TargetAim.HighlightOutlineColor
            end
        end)
    end)
end
...
RunService.Heartbeat:Connect(function()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        local hl = plr.Character and plr.Character:FindFirstChild("SolvexisHighlight")
        if hl and (not target or not solvexis_cfg.TargetAim.Highlight or plr ~= target) then
            hl:Destroy()
    ...
    if not solvexis_cfg.TargetAim.Highlight or not target or not target.Character then return end

    local hum = target.Character:FindFirstChild("Humanoid")
    if not hum then return end
    
    local highlight = target.Character:FindFirstChild("SolvexisHighlight")
    if not highlight then
        highlight = Instance.new("Highlight")
        highlight.Name = "SolvexisHighlight"
...
TargetAimTab:AddToggle('DotCircle', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.DotCircle = Value
        DotCircle.Visible = Value and solvexis_cfg.TargetAim.Enabled
    end
...
TargetAimTab:AddToggle('Tracer', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.Tracer = Value
        tracer.Visible = false
        tracerOutline.Visible = false
    end
}):AddColorPicker('TracerFill', {
    Default = solvexis_cfg.TargetAim.TracerFillColor,
    Title = 'Fill Color',
    Callback = function(Value)
        solvexis_cfg.TargetAim.TracerFillColor = Value
        tracer.Color = Value
    end
}):AddColorPicker('TracerOutline', {
    Default = solvexis_cfg.TargetAim.TracerOutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        solvexis_cfg.TargetAim.TracerOutlineColor = Value
...
TargetAimTab:AddDropdown('TracerPosition', {
    ...
    Callback = function(Value)
        solvexis_cfg.TargetAim.TracerPosition = Value
    end
})
...
TargetAimTab:AddToggle('LookAt', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.LookAt = Value
    end
})
TargetAimTab:AddButton('Teleport to Target', function()
    local targetName = solvexis_cfg.TargetAim.Target  -- Sử dụng target từ TargetAim
    if not targetName or targetName == "" or targetName == "None" then
        Library:Notify("Solvexis Hub - No target from TargetAim!", 3)
        return
    end
    local targetPlayer = Players:FindFirstChild(targetName)
    if not targetPlayer or not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        Library:Notify("Solvexis Hub - Target not found or no character!", 3)
        return
    end
    local hrp = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then
        Library:Notify("Solvexis Hub - Your character not loaded!", 3)
        return
    end
...
TargetAimTab:AddToggle('AutoKillEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.AutoKill = Value
    end
...
TargetAimTab:AddToggle('AutoKillEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.Spoofer = Value
    end
})
TargetAimTab:AddToggle('ToggleStrafe', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.ToggleStrafe = Value
    end
    ...
    Mode = 'Toggle',
    Callback = function(Value)
        if solvexis_cfg.TargetAim.ToggleStrafe then
            solvexis_cfg.TargetAim.Strafe = Value
            if Value then
...
TargetAimTab:AddToggle('VisualizeStrafe', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.VisualizeStrafe = Value
    end
}):AddColorPicker('VisInline', {
    Default = solvexis_cfg.TargetAim.VisualizeStrafeInlineColor,
    Title = 'Inline Color',
    Callback = function(Value)
        solvexis_cfg.TargetAim.VisualizeStrafeInlineColor = Value
        BodyCloneHighlight.FillColor = Value
    end
}):AddColorPicker('VisOutline', {
    Default = solvexis_cfg.TargetAim.VisualizeStrafeOutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        solvexis_cfg.TargetAim.VisualizeStrafeOutlineColor = Value
...
TargetAimTab:AddDropdown('StrafeMethod', {
    ...
    Text = 'Strafe Method',
    Callback = function(Value)
        solvexis_cfg.TargetAim.StrafeMethod = Value
    end
})
...
TargetAimTab:AddSlider('StrafePrediction', {
    ...
    Rounding = 2,
    Callback = function(Value)
        solvexis_cfg.TargetAim.StrafePrediction = Value
    end
})
...
TargetAimTab:AddToggle('AutoStomp', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.AutoStomp = Value
    end
})
...
TargetAimTab:AddToggle('SpectateTarget', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.SpectateTarget = Value

        if Value then
            if solvexis_cfg.TargetAim.Target == "None" then
                Library:Notify(
                    "Solvexis Hub - please select a target before using 'Spectate Target'.",
                    3
                )
                Toggles.SpectateTarget.Value = false
                solvexis_cfg.TargetAim.SpectateTarget = false
                return
            end
...
HitEffectsGroup:AddToggle('HitSkeleton', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitSkeleton.Enabled = Value
    end
}):AddColorPicker('HitSkeletonColor', {
    Default = Color3.fromRGB(255, 255, 255),
    Title = 'Skeleton Color',
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitSkeleton.Color = Value
    end
})
HitEffectsGroup:AddToggle('HitChamsEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitChams.Enabled = Value
    end
}):AddColorPicker('HitChamsColor', {
    Default = solvexis_cfg.HitEffects.HitChams.Color,
    Title = 'Hit Chams Color',
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitChams.Color = Value
    end
})
...
HitEffectsGroup:AddSlider('HitChamsLifetime', {
    ...
    Min = 1,
    Max = 10,
    Default = solvexis_cfg.HitEffects.HitChams.Lifetime,
    Rounding = 1,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitChams.Lifetime = Value
    end
})
...
HitEffectsGroup:AddSlider('HitChamsTransparency', {
    ...
    Min = 0,
    Max = 1,
    Default = solvexis_cfg.HitEffects.HitChams.Transparency,
    Rounding = 2,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitChams.Transparency = Value
    end
})

HitEffectsGroup:AddDropdown('HitChamsMaterial', {
    Values = {'Neon', 'ForceField'},
    Default = solvexis_cfg.HitEffects.HitChams.Material == "Neon" and 1 or 2,
    Multi = false,
    Text = 'Hit Chams Material',
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitChams.Material = Value
    end
})
...
HitEffectsGroup:AddToggle('HitEffectEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitEffect.Enabled = Value
    end
}):AddColorPicker('HitEffectColor', {
    Default = solvexis_cfg.HitEffects.HitEffect.Color,
    Title = 'Effect Color',
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitEffect.Color = Value
    end
})
...
HitEffectsGroup:AddDropdown('HitEffectType', {
    ...
    Text = 'Effect Type',
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitEffect.Type = Value
    end
})
...
local function TriggerHitEffect(Player)
    if not solvexis_cfg.HitEffects.HitEffect.Enabled then return end
    if Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        local HitPart = Player.Character.HumanoidRootPart  -- Hoặc solvexis_cfg.TargetAim.HitPart nếu có
        local EffectType = solvexis_cfg.HitEffects.HitEffect.Type
        local EffectColor = solvexis_cfg.HitEffects.HitEffect.Color
...
HitEffectsGroup:AddToggle('HitNotifications', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitNotifications = Value
    end
})
...
HitEffectsGroup:AddSlider('NotifyTime', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitNotificationsTime = Value
    end
})
...
HitEffectsGroup:AddToggle('HitSounds', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitSounds = Value
    end
})
...
HitEffectsGroup:AddDropdown('HitSoundSelect', {
    ...
    Text = 'Hit Sound',
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitSoundID = hitsounds[Value]
    end
})
...
HitEffectsGroup:AddSlider('HitSoundVolume', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.HitEffects.HitSoundVolume = Value
    end
})
...
ChecksTab:AddToggle('CheckWall', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Checks.Wall = Value
    end
})
...
ChecksTab:AddToggle('CheckForcefield', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Checks.Forcefield = Value
    end
})
...
ChecksTab:AddToggle('CheckAlive', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Checks.Alive = Value
    end
})
...
ChecksTab:AddToggle('CheckTeam', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Checks.Team = Value
    end
})
...
OptionsTab:AddInput('PredictionInput', {
    ...
    Placeholder = '0.0000',
    Callback = function(Value)
        solvexis_cfg.TargetAim.Prediction = tonumber(Value) or 0
    end
})
...
OptionsTab:AddToggle('AutoPredictToggle', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.AutoPredict = Value
    end
})
...
OptionsTab:AddDropdown('PredictModeDropdown', {
    ...
    Callback = function(Value)
        solvexis_cfg.TargetAim.PredictMode = Value
    end
})
...
OptionsTab:AddDropdown('HitPartDropdown', {
    ...
    Text = 'Hit Part',
    Callback = function(Value)
        solvexis_cfg.TargetAim.HitPart = Value
    end
})
...
OptionsTab:AddInput('OffsetInput', {
    ...
    Placeholder = '0',
    Callback = function(Value)
        solvexis_cfg.TargetAim.Offset = tonumber(Value) or 0
    end
})
...
OptionsTab:AddInput('JumpOffsetInput', {
    ...
    Placeholder = '0',
    Callback = function(Value)
        solvexis_cfg.TargetAim.JumpOffset = tonumber(Value) or 0
    end
})
...
OptionsTab:AddToggle('AirPartToggle', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.AirPartEnabled = Value
    end
})
...
OptionsTab:AddDropdown('AirPartDropdown', {
    ...
    Text = 'Airshot Part',
    Callback = function(Value)
        solvexis_cfg.TargetAim.AirPart = Value
    end
})
...
OptionsTab:AddToggle('ResolverToggle', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.TargetAim.Resolver = Value
    end
})
...
KillAuraGroup:AddToggle('KillAuraEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.KillAura.Enabled = Value
        if not Value then
            solvexis_cfg.KillAura.Active = false
            ka_tracer.Transparency = 1
        end
    end
    ...
    Mode = 'Toggle',
    Callback = function(Value)
        if solvexis_cfg.KillAura.Enabled then
            solvexis_cfg.KillAura.Active = Value
        end
    end
})
...
KillAuraGroup:AddSlider('KillAuraRange', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.KillAura.Range = Value
    end
})
...
KillAuraGroup:AddToggle('KillAuraSilent', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.KillAura.Silent = Value
    end
})
...
KillAuraGroup:AddToggle('KillAuraVisualize', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.KillAura.Visualize = Value
    end
...
KillAuraGroup:AddToggle('StompAura', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.KillAura.StompAura = Value
    end
})
local GunModsGroup = Tabs.Main:AddRightGroupbox('Gun Mods')
solvexis_cfg.EquipAllGuns = {
    Enabled = false,
}
...
local function setupEquipAll()
    local char = localPlayer.Character
    local backpack = localPlayer.Backpack
    if not char or not backpack or not solvexis_cfg.EquipAllGuns.Enabled then return end
    ...
    EquipAllConnection = char.ChildRemoved:Connect(function(child)
        if solvexis_cfg.EquipAllGuns.Enabled and child:IsA("Tool") and child:FindFirstChild("Ammo") and (tick() - lastEquipTime) >= 0.5 then
            lastEquipTime = tick()
            task.spawn(function()
                task.wait(0.1)
                equipAllGuns()
    ...
    EquipAddedConnection = backpack.ChildAdded:Connect(function(child)
        if solvexis_cfg.EquipAllGuns.Enabled and child:IsA("Tool") and child:FindFirstChild("Ammo") then
            child.Parent = char
        end
    end)
end
...
localPlayer.CharacterAdded:Connect(function()
    task.wait(5)
    if solvexis_cfg.EquipAllGuns.Enabled then
        setupEquipAll()
    end
end)
...
GunModsGroup:AddToggle('EquipAllGunsEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.EquipAllGuns.Enabled = Value
        if Value then
            setupEquipAll()
        else
            if EquipAllConnection then
...
GunModsGroup:AddToggle('RapidFireEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.RapidFire.Enabled = Value
...
GunModsGroup:AddToggle('WallbangEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Wallbang.Enabled = Value
        if getnamecallmethod then
...
HitboxGroup:AddToggle('HitboxEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.HitboxExpander.Enabled = Value
    end
})
...
HitboxGroup:AddToggle('HitboxVisualize', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.HitboxExpander.Visualize = Value
    end
}):AddColorPicker('HitboxColor', {
    Default = solvexis_cfg.HitboxExpander.Color,
    Title = 'Fill Color',
    Callback = function(Value)
        solvexis_cfg.HitboxExpander.Color = Value
    end
}):AddColorPicker('HitboxOutline', {
    Default = solvexis_cfg.HitboxExpander.OutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        solvexis_cfg.HitboxExpander.OutlineColor = Value
    end
})
...
HitboxGroup:AddSlider('HitboxFillTrans', {
    ...
    Rounding = 2,
    Callback = function(Value)
        solvexis_cfg.HitboxExpander.FillTransparency = Value
    end
})
...
HitboxGroup:AddSlider('HitboxOutlineTrans', {
    ...
    Rounding = 2,
    Callback = function(Value)
        solvexis_cfg.HitboxExpander.OutlineTransparency = Value
    end
})
...
HitboxGroup:AddSlider('HitboxSize', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.HitboxExpander.Size = Value
    end
})

getgenv().Solvexis.Legit = {
    Aimlock = false,
    HitPart = "Head",
    Smoothing = false,
    SmoothingAmount = 0.1,
...
getgenv().Solvexis.Legit.Triggerbot.HitParts = {
...
getgenv().Solvexis.Legit.Triggerbot.SelectedHitParts = {
    "Head", "HumanoidRootPart", "UpperTorso", "LowerTorso",
    "LeftUpperArm", "RightUpperArm", "LeftLowerArm", "RightLowerArm",
    "LeftHand", "RightHand", "LeftUpperLeg", "RightUpperLeg",
    "LeftLowerLeg", "RightLowerLeg", "LeftFoot", "RightFoot",
...
TriggerBox:AddToggle('TriggerbotEnabled', {
    Text = 'Enabled',
    Default = false,
    Callback = function(v) getgenv().Solvexis.Legit.Triggerbot.Enabled = v end
}):AddKeyPicker('TargetAimKey', {
    Default = 'none',
    Text = 'Target Aim',
    Mode = 'Toggle',
    Callback = function(Value) getgenv().Solvexis.Legit.Triggerbot.Enabled = Value end 
})
TriggerBox:AddToggle('TriggerbotOnlyTarget', {
    Text = 'Only Target',
    Default = false,
    Tooltip = 'Chỉ bắn khi target đang được aimbot lock',
    Callback = function(v) getgenv().Solvexis.Legit.Triggerbot.OnlyTarget = v end
})
...
TriggerBox:AddSlider('TriggerbotFOV', {
    ...
    Default = 20,
    Rounding = 1,
    Callback = function(v) getgenv().Solvexis.Legit.Triggerbot.FOV = v end
})
TriggerBox:AddDropdown('TriggerbotHitParts', {
    ...
    Default = getgenv().Solvexis.Legit.Triggerbot.HitParts, -- chọn hết mặc định
    Multi = true,
    Callback = function(selected)
        getgenv().Solvexis.Legit.Triggerbot.SelectedHitParts = selected
    end
})
TriggerBox:AddSlider('TriggerbotDelay', {
    ...
    Default = 0,
    Rounding = 2,
    Callback = function(v) getgenv().Solvexis.Legit.Triggerbot.Delay = v end
})
...
TriggerBox:AddDropdown('TriggerbotChecks', {
    ...
    Callback = function(selected)
        getgenv().Solvexis.Legit.Triggerbot.CheckKO     = table.find(selected, "KO")      ~= nil
        getgenv().Solvexis.Legit.Triggerbot.CheckKnife  = table.find(selected, "Knife")   ~= nil
        getgenv().Solvexis.Legit.Triggerbot.CheckGrab   = table.find(selected, "Grab")    ~= nil
        getgenv().Solvexis.Legit.Triggerbot.CheckTeam   = table.find(selected, "Team")    ~= nil
        getgenv().Solvexis.Legit.Triggerbot.CheckFriend = table.find(selected, "Friend")  ~= nil
    end
})
...
local function isValidTriggerTarget(plr)
    ...
    if getgenv().Solvexis.Legit.Triggerbot.OnlyTarget then
        if not solvexis_cfg.TargetAim.Target or plr.Name ~= solvexis_cfg.TargetAim.Target then
            return false
        end
    end

    if getgenv().Solvexis.Legit.Triggerbot.CheckTeam and plr.Team == LocalPlayer.Team then return false end
    if getgenv().Solvexis.Legit.Triggerbot.CheckFriend and LocalPlayer:IsFriendsWith(plr.UserId) then return false end
    if getgenv().Solvexis.Legit.Triggerbot.CheckKO and isKO(plr) then return false end
    if getgenv().Solvexis.Legit.Triggerbot.CheckGrab and isGrabbed(plr) then return false end
    if getgenv().Solvexis.Legit.Triggerbot.CheckKnife and isHoldingKnife() then return false end
    return true
end
local function GetBestTriggerPart()
    local bestPart, bestDist = nil, getgenv().Solvexis.Legit.Triggerbot.FOV

    for _, plr in Players:GetPlayers() do
        if isValidTriggerTarget(plr) and plr.Character then
            for _, partName in ipairs(getgenv().Solvexis.Legit.Triggerbot.SelectedHitParts) do
...
RunService.RenderStepped:Connect(function()
    local trigCfg = getgenv().Solvexis.Legit.Triggerbot
...
SilentBox:AddToggle('SilentAimEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().Solvexis.Legit.SilentAim = Value
    end
})
...
SilentBox:AddDropdown('SilentAimMethod', {
    ...
    Callback = function(Value)
        getgenv().Solvexis.Legit.SilentAimMethod = Value
    end
})
SilentBox:AddToggle('SilentAimEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().Solvexis.Legit.SilentFOVEnabled = Value
    end
})
SilentBox:AddSlider('SilentFOV', {
    ...
    Rounding = 0,
    Callback = function(Value)
        getgenv().Solvexis.Legit.SilentFOV = Value
    end
})
local camera = workspace.CurrentCamera
...
local function getClosestPlayerToMouse()  -- Hàm closest player từ code Rival
    local closestPlayer = nil
    local shortestDistance = getgenv().Solvexis.Legit.SilentFOVEnabled and getgenv().Solvexis.Legit.SilentFOV or 9999
...
RunService.Heartbeat:Connect(function()
    if getgenv().Solvexis.Legit.SilentAim and getgenv().Solvexis.Legit.SilentAimMethod == "Rival" and not isLobbyVisible() then
...
spawn(function()
    ...
    RunService.Heartbeat:Connect(function()
        if not getgenv().Solvexis.Legit.SilentAim or getgenv().Solvexis.Legit.SilentAimMethod ~= "Flick" then
        ...
        if not BulletHandler then
            ...
            if BulletHandler and BulletHandler.Fire then
                ...
                BulletHandler.Fire = newcclosure(function(data, ...)
                    if not getgenv().Solvexis.Legit.SilentAim then
                        return OldFire(data, ...)
                    end

                    local closest = get_closest_target(getgenv().Solvexis.Legit.SilentFOVEnabled and getgenv().Solvexis.Legit.SilentFOV or 9999)
...
AimlockBox:AddToggle('AimlockEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().Solvexis.Legit.Aimlock = Value
    end
})
AimlockBox:AddDropdown('HitPart', {
    ...
    Text = 'Hit Part',
    Callback = function(Value)
        getgenv().Solvexis.Legit.HitPart = Value
    end
})
AimlockBox:AddToggle('SmoothingEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().Solvexis.Legit.Smoothing = Value
    end
})
AimlockBox:AddSlider('SmoothingAmount', {
    ...
    Rounding = 2,
    Callback = function(Value)
        getgenv().Solvexis.Legit.SmoothingAmount = Value
    end
})
AimlockBox:AddSlider('Prediction', {
    ...
    Rounding = 2,
    Callback = function(Value)
        getgenv().Solvexis.Legit.Prediction = Value
    end
})
AimlockBox:AddSlider('JumpOffset', {
    ...
    Rounding = 1,
    Callback = function(Value)
        getgenv().Solvexis.Legit.JumpOffset = Value
    end
})
AimlockBox:AddToggle('ResolverToggle', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().Solvexis.Legit.Resolver = Value
    end
})
local VelocityData = {}
...
local function updateCamlock()
    local targetName = solvexis_cfg.TargetAim.Target
    if not getgenv().Solvexis.Legit.Aimlock then return end
    ...
    local part = target.Character:FindFirstChild(getgenv().Solvexis.Legit.HitPart)
    ...
    if getgenv().Solvexis.Legit.Resolver and VelocityData[target] then
        velocity = VelocityData[target].Velocity
    end
    
    local pred = getgenv().Solvexis.Legit.Prediction or 0
    local pos = part.Position + (velocity * pred)
    
    --// JUMP OFFSET (nếu bật và đang rơi/jump)
    if getgenv().Solvexis.Legit.Offset and target.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
        pos = pos + Vector3.new(0, getgenv().Solvexis.Legit.JumpOffset or 0, 0)
    end
    
    local goal = CFrame.new(Camera.CFrame.Position, pos)
    
    if getgenv().Solvexis.Legit.Smoothing then
        Camera.CFrame = Camera.CFrame:Lerp(goal, getgenv().Solvexis.Legit.SmoothingAmount)
    else
        Camera.CFrame = goal
    end
end
RunService.RenderStepped:Connect(function()
    if getgenv().Solvexis.Legit.Aimlock then
        updateCamlock()
    end
end)
...
local SolvexisEsp = loadstring(game:HttpGet('https://raw.githubusercontent.com/alebinh60/asmobile/refs/heads/main/esplib'))()
local esp = SolvexisEsp
...
local function bullettracerlol(startPos, endPos)
    ...
    beam.Color = ColorSequence.new(solvexis_cfg.BulletTracers.Color)
    beam.Texture = solvexis_cfg.BulletTracers.TextureID
    beam.LightEmission = 1
    beam.Transparency = NumberSequence.new(solvexis_cfg.BulletTracers.Transparency)
    beam.Width0 = solvexis_cfg.BulletTracers.Size
    beam.Width1 = solvexis_cfg.BulletTracers.Size

    task.delay(solvexis_cfg.BulletTracers.TimeAlive, function()
        ...
        __namecall = (function(self, ...)
            ...
            if getnamecallmethod() == "FireServer" then
                if args[1] == "ShootGun" then
                    ...
                    -- Bullet Tracers
                    if solvexis_cfg.BulletTracers.Enabled then
...
BulletTracerGroup:AddToggle('BulletTracersEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.BulletTracers.Enabled = Value
        if not getnamecallmethod then
            Library:Notify("Your executor does not support this feature")
        end
    end
}):AddColorPicker('BulletTracersColor', {
    Default = solvexis_cfg.BulletTracers.Color,
    Title = 'Color',
    Callback = function(Value)
        solvexis_cfg.BulletTracers.Color = Value
    end
})
...
BulletTracerGroup:AddDropdown('BulletTracersTexture', {
    ...
    Callback = function(Value)
        if Value == "Beam" then
            solvexis_cfg.BulletTracers.TextureID = "rbxassetid://12781852245"
        elseif Value == "Lightning" then
            solvexis_cfg.BulletTracers.TextureID = "rbxassetid://446111271"
        elseif Value == "Heartrate" then
            solvexis_cfg.BulletTracers.TextureID = "rbxassetid://5830549480"
        elseif Value == "Chain" then
            solvexis_cfg.BulletTracers.TextureID = "rbxassetid://9632168658"
        elseif Value == "Glitch" then
            solvexis_cfg.BulletTracers.TextureID = "rbxassetid://8089467613"
        elseif Value == "Swirl" then
            solvexis_cfg.BulletTracers.TextureID = "rbxassetid://5638168605"
        end
    end
})
...
BulletTracerGroup:AddSlider('BulletTracersSize', {
    ...
    Rounding = 2,
    Callback = function(Value)
        solvexis_cfg.BulletTracers.Size = Value
    end
})
...
BulletTracerGroup:AddSlider('BulletTracersTransparency', {
    ...
    Rounding = 2,
    Callback = function(Value)
        solvexis_cfg.BulletTracers.Transparency = Value
    end
})
...
BulletTracerGroup:AddSlider('BulletTracersTimeAlive', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.BulletTracers.TimeAlive = Value
    end
})
...
RunService.RenderStepped:Connect(function(deltaTime)
    ...
    if isFollowTargetEnabled and solvexis_cfg and solvexis_cfg.TargetAim and solvexis_cfg.TargetAim.Target ~= "None" then
        
        local targetPlayer = Players:FindFirstChild(solvexis_cfg.TargetAim.Target)
...
MovementGroup:AddToggle('SpeedEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Movement.Speed.Enabled = Value
    end
    ...
    Text = 'Speed Key',
    Callback = function(Value)
        solvexis_cfg.Movement.Speed.Keybind = Value
    end
})
...
MovementGroup:AddSlider('SpeedValue', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.Movement.Speed.Speed = Value
    end
})
MovementGroup:AddToggle('FlyEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Movement.Fly.Enabled = Value
    end
}):AddKeyPicker('FlyKey', {
    Default = 'none',
    SyncToggleState = true,
    ...
    Text = 'Fly Key',
    Callback = function(Value)
        solvexis_cfg.Movement.Fly.Keybind = Value
    end
})
...
MovementGroup:AddSlider('FlySpeed', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.Movement.Fly.Speed = Value
    end
})
...
MovementGroup:AddToggle('WalkSpeedToggle', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Movement.WalkSpeedEnabled = Value
    end
...
MovementGroup:AddSlider('WalkSpeedSlider', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.Movement.WalkSpeed = Value
    end
})
...
MovementGroup:AddToggle('JumpPowerToggle', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Movement.JumpPowerEnabled = Value
    end
...
MovementGroup:AddSlider('JumpPowerSlider', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.Movement.JumpPower = Value
    end
})

getgenv().Solvexis.BunnyHop = {
    Enabled = false,
    Keybind = false,
    Speed = 100
}
MovementGroup:AddToggle('BunnyHop_Enabled', {
    ...
    Default = false,
    Callback = function(value)
        getgenv().Solvexis.BunnyHop.Enabled = value
    end
    ...
    Callback = function(state)
        getgenv().Solvexis.BunnyHop.Keybind = state
    end
})
...
MovementGroup:AddSlider('BunnyHop_Speed', {
    ...
    Rounding = 0,
    Callback = function(value)
        getgenv().Solvexis.BunnyHop.Speed = value
    end
})

-- Logic Bunny Hop (chạy liên tục)
RunService.RenderStepped:Connect(function()
    if not getgenv().Solvexis.BunnyHop.Enabled or not getgenv().Solvexis.BunnyHop.Keybind then return end
...
MovementGroup:AddToggle('SpinBot_Enabled', {
    ...
    Default = false,
    Callback = function(value)
        getgenv().Solvexis.SpinBot.Enabled = value
    end
...
MovementGroup:AddSlider('SpinBot_Speed', {
    ...
    Rounding = 0,
    Callback = function(value)
        getgenv().Solvexis.SpinBot.Speed = value
    end
})
...
game.Players.LocalPlayer.Character.Humanoid.Health = 0
RunService.Heartbeat:Connect(function(dt)
    if not getgenv().Solvexis.SpinBot.Enabled then
    ...
    local spinSpeed = getgenv().Solvexis.SpinBot.Speed or 300  
...
RunService.Heartbeat:Connect(function()
    ...
    if hum then
        if solvexis_cfg.Movement.WalkSpeedEnabled then
            hum.WalkSpeed = solvexis_cfg.Movement.WalkSpeed
        end
        if solvexis_cfg.Movement.JumpPowerEnabled then
            hum.JumpPower = solvexis_cfg.Movement.JumpPower
...
localPlayer.CharacterAdded:Connect(function(char)
    ...
    if solvexis_cfg.Movement.FlyEnabled then StartFly() end
end)
-- Integrate Movement Logic into Heartbeat
RunService.Heartbeat:Connect(function(dt)
    -- Speed
    if solvexis_cfg.Movement.Speed.Enabled and solvexis_cfg.Movement.Speed.Keybind then
        if localPlayer and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local move_direction = localPlayer.Character.Humanoid.MoveDirection
            local hrp = localPlayer.Character.HumanoidRootPart
            hrp.CFrame = hrp.CFrame + (move_direction * dt) * solvexis_cfg.Movement.Speed.Speed * 10
        end
    end
    
    -- Fly
    if solvexis_cfg.Movement.Fly.Enabled and solvexis_cfg.Movement.Fly.Keybind then
        if localPlayer and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local move_direction = localPlayer.Character.Humanoid.MoveDirection
            local hrp = localPlayer.Character.HumanoidRootPart
            local add = Vector3.new(0, (UserInputService:IsKeyDown(Enum.KeyCode.Space) and solvexis_cfg.Movement.Fly.Speed / 8 or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and -solvexis_cfg.Movement.Fly.Speed / 8) or 0, 0)
            hrp.CFrame = hrp.CFrame + (move_direction * dt) * solvexis_cfg.Movement.Fly.Speed * 10
...
RunService.Heartbeat:Connect(function(delta)
    if not solvexis_cfg.TargetAim.Enabled or solvexis_cfg.TargetAim.Target == "None" then
        DotCircle.Visible = false
        return
    end
    ...
    local hitPart = targetPlayer.Character:FindFirstChild(solvexis_cfg.TargetAim.HitPart)
    if not hitPart then
        DotCircle.Visible = false
        return
    end
    ...
    -- Resolver
    if solvexis_cfg.TargetAim.Resolver then
    ...
    local predictVal = solvexis_cfg.TargetAim.AutoPredict and solvexis_cfg.TargetAim.PredictMode == "Ping" and (Stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000 + 0.035) or solvexis_cfg.TargetAim.Prediction
    local predictedPos = hitPart.Position + hitPart.Velocity * predictVal
    local screenPos, onScreen = Camera:WorldToViewportPoint(predictedPos)

    if onScreen and solvexis_cfg.TargetAim.DotCircle then
	    ...
	    for _, tool in pairs(char:GetChildren()) do
	        if tool:IsA("Tool") then
	            tool.Activated:Connect(function()
	                if not solvexis_cfg.TargetAim.Enabled or solvexis_cfg.TargetAim.Target == "None" or not CurrentGame then return end
	                ...
	                local predictVal = solvexis_cfg.TargetAim.Prediction
	                if solvexis_cfg.TargetAim.AutoPredict then
	                    if solvexis_cfg.TargetAim.PredictMode == "Ping" then
	                        predictVal = (game.Stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000) + 0.035
	                    elseif solvexis_cfg.TargetAim.PredictMode == "Calculate" then
...
if getnamecallmethod and not isMobile then
    ...
    Meta.__index = function(t, k)
        if t == Mouse and (k == "Hit" or k == "hit") then
            if solvexis_cfg.TargetAim.Enabled and solvexis_cfg.TargetAim.Target ~= "None" and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local TargetPlayer = Players:FindFirstChild(solvexis_cfg.TargetAim.Target)
                if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Humanoid") and TargetPlayer.Character.Humanoid.Health > 0 and not TargetPlayer.Character:FindFirstChild("ForceField") then
                    ...
                    -- Team check
                    if solvexis_cfg.Checks.Team and TargetPlayer.Team == LocalPlayer.Team then return backupindex(t, k) end
                    ...
                    local partName = solvexis_cfg.TargetAim.HitPart
                    if solvexis_cfg.TargetAim.AirPartEnabled then
                        ...
                        if targetInAir or selfInAir then
                            partName = solvexis_cfg.TargetAim.AirPart
                        end
                    end
                    ...
                    if targetPart then
                        ...
                        -- Prediction logic
                        local pred = solvexis_cfg.TargetAim.Prediction -- manual default
                        if solvexis_cfg.TargetAim.AutoPredict then
                            if solvexis_cfg.TargetAim.PredictMode == "Ping Sets" then
                                pred = currentPred
                            elseif solvexis_cfg.TargetAim.PredictMode == "Calculate" then
...
if getnamecallmethod and game.GameId == 9825515356 then
    ...
    OldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if method == "FireServer" and self.Name == "MainEvent" and args[1] == "Shoot" then
            if solvexis_cfg.TargetAim.Enabled and solvexis_cfg.TargetAim.AutoFire and solvexis_cfg.TargetAim.Target ~= "None" and localPlayer and localPlayer.Character then
                local TargetPlayer = players:FindFirstChild(solvexis_cfg.TargetAim.Target)
...
players.PlayerRemoving:Connect(function(plr)
    if plr.Name == solvexis_cfg.TargetAim.Target then
...
RunService.Heartbeat:Connect(function(dt)
    pcall(function()
        if getnamecallmethod and solvexis_cfg.RapidFire.Enabled then
        ...
        local useDesync = solvexis_cfg.TargetAim.Strafe or solvexis_cfg.TargetAim.AutoStomp or solvexis_cfg.KillAura.StompAura or solvexis_cfg.TargetAim.AutoKill or headshots.AutoLoadout.Enabled or BuyingSingle or BuyingAmmoActive
        local TargetPlayer = players:FindFirstChild(solvexis_cfg.TargetAim.Target)
        ...
        if useDesync and hrp then
            if solvexis_cfg.TargetAim.Strafe and solvexis_cfg.TargetAim.Target ~= "None" and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Head") and not (AutoKillActive or AutoArmorActive or AutoLoadoutActive or BuyingSingleActive or AutoHealActive) then
                if not TargetPlayer.Character:FindFirstChild("ForceField") then
                    if not KnockCheck(TargetPlayer) then
                        ...
                        local strafeOffset
                        if solvexis_cfg.TargetAim.StrafeMethod == "Orbit" then
                            strafeOffset = Vector3.new(math.cos(tick()*10)*10, 0, math.sin(tick()*10)*10)
                        elseif solvexis_cfg.TargetAim.StrafeMethod == "Randomize" then
                        ...
                        if solvexis_cfg.TargetAim.Spoofer then
                            RunService:BindToRenderStep("RestoreStrafe", 199, function()
                                hrp.CFrame = SavedPosition
                                RunService:UnbindFromRenderStep("RestoreStrafe")
                            end)
                        end
                        if solvexis_cfg.TargetAim.VisualizeStrafe then
                            BodyClone:SetPrimaryPartCFrame(hrp.CFrame)
                            BodyCloneHighlight.Enabled = true
                            SetRigTransparency(BodyClone, 0)
                            SetRigColor(BodyClone, solvexis_cfg.TargetAim.VisualizeStrafeInlineColor)
                            BodyCloneHighlight.FillColor = solvexis_cfg.TargetAim.VisualizeStrafeInlineColor
                            BodyCloneHighlight.OutlineColor = solvexis_cfg.TargetAim.VisualizeStrafeOutlineColor
                        else
                            BodyCloneHighlight.Enabled = false
                        ...
                        if solvexis_cfg.TargetAim.AutoFire and MainEvent then
            ...
            if solvexis_cfg.TargetAim.Enabled and solvexis_cfg.TargetAim.Target ~= "None" and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Head") and not KnockCheck(TargetPlayer) then
                local tool = localPlayer.Character:FindFirstChildOfClass("Tool")
                if tool and not TargetPlayer.Character:FindFirstChild("ForceField") then
                    if solvexis_cfg.TargetAim.AutoFire then
...
RunService.Heartbeat:Connect(function(dt)
	...
	local useDesync = solvexis_cfg.TargetAim.Strafe or solvexis_cfg.TargetAim.AutoStomp or solvexis_cfg.KillAura.StompAura or solvexis_cfg.TargetAim.AutoKill or headshots.AutoLoadout.Enabled or BuyingSingle or BuyingAmmo
	...
	if solvexis_cfg.TargetAim.Enabled and solvexis_cfg.TargetAim.Target ~= "None" then
	    local TargetPlayer = players:FindFirstChild(solvexis_cfg.TargetAim.Target)
	    if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Humanoid") then
	        ...
	        if prev ~= nil and curr < prev then
	            -- Hit Sound
	            if solvexis_cfg.HitEffects.HitSounds then
	                createHitSound()
	            end
	
	            -- Hit Notification
	            if solvexis_cfg.HitEffects.HitNotifications then
	                Library:Notify("Solvexis Hub - Target Hit: " .. TargetPlayer.Name .. " - Health: " .. curr, solvexis_cfg.HitEffects.HitNotificationsTime)
	            end
	
	            -- Hit Chams
	            if solvexis_cfg.HitEffects.HitChams.Enabled then
	                HitChams(TargetPlayer)
	            end
                if solvexis_cfg.HitEffects.HitEffect.Enabled then
                    TriggerHitEffect(TargetPlayer)
                end
                if solvexis_cfg.HitEffects.HitSkeleton.Enabled then
    ...
    if solvexis_cfg.KillAura.Enabled then
        if Tool and Tool:FindFirstChild("Handle") then
			...
			local ka_target = nil
			for _, player in pairs(players:GetPlayers()) do
				if player ~= localPlayer and not solvexis_cfg.KillAura.Whitelist[player.Name] and player.Character and player.Character:FindFirstChild("Head") and not player.Character:FindFirstChild("GRABBING_CONSTRAINT") then
					if player.Character.BodyEffects:FindFirstChild("K.O") and not player.Character.BodyEffects["K.O"].Value then
						...
						if dist < closest and dist <= solvexis_cfg.KillAura.Range then
							closest = dist
							ka_target = player
						end
					end
			...
			if ka_target and ka_target.Character and ka_target.Character:FindFirstChild("Head") then
				...
				if solvexis_cfg.KillAura.Visualize then
				...
				local offset = solvexis_cfg.KillAura.Silent and Vector3.new(0, -12, 0) or Vector3.new(0, 0, 0)
    ...
    if solvexis_cfg.TargetAim.Enabled and solvexis_cfg.TargetAim.Target ~= "None" then
        ...
        if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Humanoid") and TargetPlayer.Character:FindFirstChild("Head") and not TargetPlayer.Character:FindFirstChild("ForceField") then
            ...
            if humanoid then
                ...
                if lastHealth == nil or solvexis_cfg.TargetAim.Target ~= lastTarget then
                    lastHealth = currentHealth
                end
                if currentHealth < lastHealth then
                    if solvexis_cfg.HitEffects.HitNotifications then
                        Library:Notify("Solvexis Hub - Target Hit: " .. TargetPlayer.Name .. " - Health: " .. currentHealth, solvexis_cfg.HitEffects.HitNotificationsTime)
                    end
                    if solvexis_cfg.HitEffects.HitSounds then
                        createHitSound()
                    end
                ...
                lastHealth = currentHealth
                lastTarget = solvexis_cfg.TargetAim.Target
            end
        end
    end
    if solvexis_cfg.TargetAim.Enabled and solvexis_cfg.TargetAim.Target ~= "None" and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Head") then
        if solvexis_cfg.TargetAim.LookAt then
        ...
        if solvexis_cfg.TargetAim.Tracer then
            ...
            if head and humanoidRootPart then
                ...
                if onScreen then
                    ...
                    local tool = localPlayer.Character:FindFirstChildOfClass("Tool")
                    if solvexis_cfg.TargetAim.TracerPosition == "Tool" and tool and tool:FindFirstChild("Handle") then
    ...
    if solvexis_cfg.TargetAim.Target ~= "None" then
        ...
        if TargetPlayer then
            ...
            Library:Notify(solvexis_cfg.TargetAim.Target .. " left the game", 3)
            solvexis_cfg.TargetAim.Target = "None"
            previousKnock = false
            previousDead = false
            lastTargetCharacter = nil
    ...
    if useDesync and hrp then
        if solvexis_cfg.TargetAim.AutoStomp and TargetPlayer and KnockCheck(TargetPlayer) and localPlayer.Character.BodyEffects.Reload.Value == false then
            ...
            if isDead == false then
                ...
                if solvexis_cfg.HitEffects.HitNotifications then
                    if tick() - t >= 3 then
                        t = tick()
                        Library:Notify("Solvexis Hub - Attempted to Stomp Target: " .. TargetPlayer.Name, solvexis_cfg.HitEffects.HitNotificationsTime)
                    end
                end
                MainEvent:FireServer("Stomp")
            end
        elseif solvexis_cfg.KillAura.StompAura and not (AutoKillActive or TargetAimActive or AutoArmorActive or AutoLoadoutActive or BuyingSingleActive or AutoHealActive) then
            local shortestDistance = math.huge
            local stompTarget
            for _, player in pairs(players:GetPlayers()) do
                if player ~= localPlayer and player.Character and player.Character:FindFirstChild("Head") and KnockCheck(player) and not player.Character:FindFirstChild("ForceField") then
                    ...
                    if distance <= solvexis_cfg.KillAura.Range and distance < shortestDistance then
                        shortestDistance = distance
                        stompTarget = player
                    end
                end
...
RunService.Heartbeat:Connect(function()
    ...
    ------------------------------------------------------------
    if solvexis_cfg.TargetAim.Enabled
        and solvexis_cfg.TargetAim.Strafe
        and solvexis_cfg.TargetAim.Target ~= "None"
        and localPlayer.Character
        and localPlayer.Character:FindFirstChild("HumanoidRootPart")
        and localPlayer.Character:FindFirstChild("Humanoid")
        and localPlayer.Character:FindFirstChild("Head") then

        local target = players:FindFirstChild(solvexis_cfg.TargetAim.Target)
        if target and target.Character and target.Character:FindFirstChild("Head")
            ...
            if ((not KnockCheck(target)) or (solvexis_cfg.TargetAim.AutoStomp and dead == false)) then
...
RunService.Heartbeat:Connect(function(dt)
    if solvexis_cfg.TargetAim.AutoKill and not (TargetAimActive or AutoArmorActive or AutoLoadoutActive or BuyingSingleActive or AutoHealActive or BuyingAmmoActive) then
        local targetName = solvexis_cfg.TargetAim.Target
...
RS.RenderStepped:Connect(function()
    if solvexis_cfg.HitboxExpander.Enabled then
        for _, Player in pairs(players:GetPlayers()) do
            if Player == Client then continue end
            local HRP = Player.Character and Player.Character:WaitForChild("HumanoidRootPart")
            if HRP then
                HRP.Size = Vector3.new(solvexis_cfg.HitboxExpander.Size, solvexis_cfg.HitboxExpander.Size, solvexis_cfg.HitboxExpander.Size)
                HRP.CanCollide = false
                if solvexis_cfg.HitboxExpander.Visualize then
                    if not highlights[Player] then
                        local Visualize = Instance.new("Highlight")
                        HRP.Transparency = 0.9
                        Visualize.Parent = HRP
                        Visualize.FillColor = solvexis_cfg.HitboxExpander.Color
                        Visualize.OutlineColor = solvexis_cfg.HitboxExpander.OutlineColor
                        Visualize.FillTransparency = solvexis_cfg.HitboxExpander.FillTransparency
                        Visualize.OutlineTransparency = solvexis_cfg.HitboxExpander.OutlineTransparency
...
local function loadDance(name)
    ...
    if animId then
        ...
        if getgenv().Solvexis.Dance.Playing then
...
AnimationGroup:AddDropdown('DanceSelect', {
    ...
    Searchable = true,
    Callback = function(v)
        getgenv().Solvexis.Dance.Selected = v
        loadDance(v)
    end
})
...
AnimationGroup:AddToggle('DanceToggle', {
    ...
    Default = false,
    Callback = function(v)
        getgenv().Solvexis.Dance.Enabled = v
        getgenv().Solvexis.Dance.Playing = v
...
getgenv().Solvexis.Character = getgenv().Solvexis.Character or {}
getgenv().Solvexis.Character.Noclip = false

local NoclipConnection
local function toggleNoclip(bool)
    getgenv().Solvexis.Character.Noclip = bool
    if bool then
...
RunService.Heartbeat:Connect(function()
    ...
    if solvexis_cfg.Desync.Enabled then
        if solvexis_cfg.Desync.Mode == "Destroy Cheaters" then
            desyncCFrame = CFrame.new(9e9, 1, 1) * oldCFrame.Rotation  -- Giữ rotation
        elseif solvexis_cfg.Desync.Mode == "Underground" then
            desyncCFrame = CFrame.new(hrp.Position - Vector3.new(0, 12, 0)) * oldCFrame.Rotation
        elseif solvexis_cfg.Desync.Mode == "Void Spam" then
        ...
        elseif solvexis_cfg.Desync.Mode == "Void" then
        ...
        elseif solvexis_cfg.Desync.Mode == "Random" then
            local amount = solvexis_cfg.Desync.RandomAmount
        ...
        elseif solvexis_cfg.Desync.Mode == "Safe Shoot" then
            desyncCFrame = CFrame.new(hrp.Position - Vector3.new(0, 5, 0)) * CFrame.Angles(math.random(0, 360), math.random(0, 360), math.rad(180))
        elseif solvexis_cfg.Desync.Mode == "Custom" then
            desyncCFrame = CFrame.new(hrp.Position - Vector3.new(solvexis_cfg.Desync.CustomX, solvexis_cfg.Desync.CustomY, solvexis_cfg.Desync.CustomZ)) * oldCFrame.Rotation
        end

        -- Apply Desync (giữ nguyên)
        hrp.CFrame = desyncCFrame
        if not solvexis_cfg.TargetAim.SpectateTarget then
	...
	elseif solvexis_cfg.TargetAim.AutoKill then
	    visualizeCFrame = killpos
	else
	    visualizeCFrame = hrp.CFrame
	end
	
	if solvexis_cfg.Desync.Visualize and solvexis_cfg.Desync.Enabled then
	...
	if solvexis_cfg.Desync.Line and solvexis_cfg.Desync.Enabled then
	...
	if solvexis_cfg.Desync.Dot and solvexis_cfg.Desync.Enabled then
	    local targetPosition = visualizeCFrame.Position
    ...
    if solvexis_cfg.Desync.Status then
        DesyncStatus.Text = "Desync: " .. (solvexis_cfg.Desync.Enabled and "TRUE" or "FALSE")
        DesyncStatus.Color = solvexis_cfg.Desync.Enabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
...
UserInputService.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 and solvexis_cfg.Desync.Status then
        local mousePos = UserInputService:GetMouseLocation()
...
DesyncGroup:AddToggle('DesyncEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Desync.Enabled = Value
        if not Value then
            resetCamera()
        end
    end
    ...
    Callback = function(Key)
        ...
        solvexis_cfg.Desync.Enabled = not solvexis_cfg.Desync.Enabled
        if not solvexis_cfg.Desync.Enabled then
            resetCamera()
        end
    end
})
...
DesyncGroup:AddDropdown('DesyncMode', {
    ...
    Text = 'Desync Mode',
    Callback = function(Value)
        solvexis_cfg.Desync.Mode = Value
    end
})
DesyncGroup:AddSlider('DesyncCustomX', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.Desync.CustomX = Value
    end
})
...
DesyncGroup:AddSlider('DesyncCustomY', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.Desync.CustomY = Value
    end
})
...
DesyncGroup:AddSlider('DesyncCustomZ', {
    ...
    Rounding = 0,
    Callback = function(Value)
        solvexis_cfg.Desync.CustomZ = Value
    end
})
DesyncGroup:AddSlider('DesyncRandomAmount', {
    ...
    Compact = false,
    Callback = function(Value)
        solvexis_cfg.Desync.RandomAmount = Value
    end
})
...
DesyncGroup:AddToggle('DesyncVisualize', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Desync.Visualize = Value
    end
...
DesyncGroup:AddToggle('DesyncLine', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Desync.Line = Value
    end
})
...
DesyncGroup:AddToggle('DesyncStatus', {
    ...
    Default = false,
    Callback = function(Value)
        solvexis_cfg.Desync.Status = Value
    end
})
...
PlayerListGroup:AddButton('Set TargetAim', function()
    if selectedPlayer then
        solvexis_cfg.TargetAim.Target = selectedPlayer.Name
...
Promo = {
    "Solvexis Hub On Top🍵",
    "Solvexis Hub so clean yall still losing to it..🍵",
    "Must be rough playing without Solvexis Hub 💔🍵",
    "cant win a hvh? maybe try /matchahvh🍵",
    "🍵if u wanna stop losing hop in /matchahvh",
    "🍵bro if u cant win just get Solvexis . cc",
    "how to win a hvh? step 1: get Solvexis . cc🍵",
    "gg/Vsnz2wfjP5🍵",
    "Solvexis Hub users eat, yall starve 🍵",
    "No Solvexis Hub? no wins. simple. 🍵",
    "You crying in chat while Solvexis Hub cooking 🍵",
    "If missing shots was a job you’d be CEO 🍵",
    "Solvexis Hub got me winning, you got excuses 🍵",
    "Keep blaming lag, we know it's skill 🍵",
    "Solvexis Hub smooth, yall movement shaky 🍵",
    "Solvexis Hub on top, your aim on vacation 🍵",
    "You fighting air while Solvexis Hub hitting heads 🍵",
    "Solvexis Hub turning bots into bosses 🍵",
    "You need Solvexis Hub more than WiFi 🍵",
    "Solvexis Hub users glide, you slide 🍵",
    "Your gameplay crying, Solvexis Hub thriving 🍵",
    "You chasing clout, Solvexis Hub chasing wins 🍵",
    "Solvexis Hub got the boost, you got excuses 🍵",
    "You pressing buttons, we pressing victory 🍵",
    "Solvexis Hub users pop off, you pop up dead 🍵",
    "Skill issue? Solvexis Hub don’t judge 🍵",
    "Solvexis Hub keeps you calm, you keep panic 🍵",
    "No Solvexis Hub? say hello to respawn 🍵",
    "Solvexis Hub players flex, you stumble 🍵",
    "Your aim wandering, Solvexis Hub laser-focused 🍵",
    "Solvexis Hub got bars, you got tears 🍵",
    "Try as you might, Solvexis Hub too strong 🍵",
    "Solvexis Hub users cook, you still reading the recipe 🍵",
    "Your shots missing, Solvexis Hub hitting 🍵",
    "Solvexis Hub above, you below 🍵",
    "We slide, you collide 🍵",
    "Solvexis Hub users shine, your screen blurry 🍵",
    "Solvexis Hub clean, your game messy 🍵",
    "You talking big, we acting bigger 🍵",
    "Solvexis Hub got moves, you got panic 🍵",
    "Without Solvexis Hub? GG for you 🍵",
    "Solvexis Hub making legends, you making mistakes 🍵",
    "You flinch, we feast 🍵",
    "Solvexis Hub players rise, you respawn 🍵",
    "We dominate, you imitate 🍵",
    "Solvexis Hub got juice, you got crumbs 🍵",
    "Solvexis Hub never sleeps, you lag 🍵",
    "You chasing shadows, we chasing wins 🍵",
    "Solvexis Hub users glide, your feet stuck 🍵",
    "No Solvexis Hub? keep dreaming 🍵",
    "Solvexis Hub built for pros, you built for bots 🍵",
    "Your keyboard crying, Solvexis Hub laughing 🍵",
    "Solvexis Hub users on top, you hitting ground 🍵",
    "You tap, we snap 🍵",
    "Solvexis Hub got focus, you got panic attacks 🍵",
    "Keep trying, we keep winning 🍵",
    "Solvexis Hub moving smooth, you jittery 🍵",
    "Solvexis Hub users dominate, you spectate 🍵",
    "You losing fast, Solvexis Hub blasting 🍵",
    "Solvexis Hub got style, your shots messy 🍵",
    "You lag, we swag 🍵",
    "Solvexis Hub players clean, you dirty 🍵",
    "Your moves weak, Solvexis Hub strong 🍵",
    "Solvexis Hub users laugh, you cry 🍵",
    "Keep missing, Solvexis Hub keep scoring 🍵",
    "You freeze, we feast 🍵",
    "Solvexis Hub got energy, you tired 🍵",
    "Solvexis Hub turning noobs into legends 🍵",
    "You flailing, we flying 🍵",
    "Solvexis Hub smooth operator, you panic button 🍵",
    "No Solvexis Hub? surrender early 🍵",
    "Solvexis Hub got tactics, you got chaos 🍵",
    "You panic, we execute 🍵",
    "Solvexis Hub users calm, you screaming 🍵",
    "Solvexis Hub gliding, you stumbling 🍵",
    "You pray, we slay 🍵",
    "Solvexis Hub on top, your score low 🍵",
    "Your shots weak, Solvexis Hub strong 🍵",
    "Solvexis Hub always moving, you stuck 🍵",
    "You spinning wheels, Solvexis Hub winning 🍵",
    "Solvexis Hub users cooking, you freezing 🍵",
    "You panic, we dominate 🍵",
    "Solvexis Hub turning losses into wins 🍵",
    "You chasing shadows, we chasing glory 🍵",
    "Solvexis Hub got juice, your hands dry 🍵",
    "You flinch, we feast 🍵",
    "Solvexis Hub players glide, you stumble 🍵",
    "Your aim shaky, Solvexis Hub steady 🍵",
    "Solvexis Hub above, you below 🍵",
    "You miss shots, we hit 🍵",
    "Solvexis Hub users rise, you respawn 🍵",
    "Your tactics weak, Solvexis Hub strong 🍵",
    "Solvexis Hub got bars, you got lag 🍵",
    "You panic, we score 🍵",
    "Solvexis Hub on top, you bottom 🍵",
    "You crying, Solvexis Hub laughing 🍵",
    "Solvexis Hub got power, you weak 🍵",
    "You tap, we dominate 🍵",
    "Solvexis Hub shining, you hiding 🍵",
    "Your game messy, Solvexis Hub clean 🍵",
    "You flail, we flex 🍵",
    "Solvexis Hub cooking, you freezing 🍵",
    "Solvexis Hub users glide, you slide 🍵",
    "Your aim wandering, Solvexis Hub laser 🍵",
    "You panic, we feast 🍵",
    "Solvexis Hub got style, your shots weak 🍵",
    "You losing, Solvexis Hub winning 🍵",
    "Solvexis Hub moving smooth, you jittery 🍵",
    "No Solvexis Hub? just GG 🍵",
    "Solvexis Hub built for winners, you built for bots 🍵",
    "Your keyboard crying, Solvexis Hub laughing 🍵",
    "You spin, we glide 🍵",
    "Solvexis Hub users on top, you bottom 🍵",
    "You press panic, we press victory 🍵",
    "Solvexis Hub got juice, you dry 🍵",
    "You flinch, we feast 🍵",
    "Solvexis Hub players dominate, you spectate 🍵",
    "You lag, we swag 🍵",
    "Solvexis Hub got tactics, you chaos 🍵",
    "No Solvexis Hub? just pray 🍵",
    "Solvexis Hub smooth operator, you panic button 🍵",
    "Your game weak, Solvexis Hub strong 🍵",
    "Solvexis Hub users clean, you messy 🍵",
    "You tap, we snap 🍵",
    "Solvexis Hub on top, your score low 🍵",
    "Your shots miss, Solvexis Hub hits 🍵",
    "Solvexis Hub always moving, you stuck 🍵",
...
ThemeManager:SetFolder('solvexis_cfg')
SaveManager:SetFolder('solvexis_cfg/dahood')
